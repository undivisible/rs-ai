...
use rusty_ai::stream::{AiStream, StreamEvent as RustyStreamEvent};
use rusty_ai::Usage;
            ...
            loop {
                match event_stream.next().await {
                    Some(Ok(anthropic_event)) => {
                        let rusty_events = map_event(anthropic_event, &mut state);
                        if !rusty_events.is_empty() {
                            let items: Vec<Result<RustyStreamEvent, AiError>> =
                                rusty_events.into_iter().map(Ok).collect();
