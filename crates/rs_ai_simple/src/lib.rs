...
//! use rs_ai_simple::*;
//!
//! let ai = rs_ai_claude("claude-sonnet-4-6")?;
//! let result = ai.generate("What is 2+2?").await?;
//! ```

use rs_ai_ai::{AiError, AiResult, GenerateOptions, LanguageModel, Prompt};
...
/// ```
pub fn rs_ai_claude(model_id: &str) -> AiResult<SimpleModel> {
...
/// # Examples
/// ```ignore
/// let ai = rs_ai_chatgpt("gpt-4o")?;
/// let response = ai.generate("Hello, world!").await?;
/// ```
pub fn rs_ai_chatgpt(model_id: &str) -> AiResult<SimpleModel> {
...
/// # Examples
/// ```ignore
/// let ai = rs_ai_gemini("gemini-2.5-flash")?;
/// let response = ai.generate("Hello, world!").await?;
/// ```
pub fn rs_ai_gemini(model_id: &str) -> AiResult<SimpleModel> {
