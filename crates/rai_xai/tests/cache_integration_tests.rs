...
fn test_model_with_prompt_cache_key() {
    ...
    let model = provider
        .grok_4()
        .with_prompt_cache_key("pk-xyz-789");
...
fn test_conversation_id_persistence() {
    ...
    assert_eq!(cache_config1.xai_conv_id, Some("shared-conv-id".to_string()));
    ...
    assert_eq!(cache_config2.xai_conv_id, Some("shared-conv-id".to_string()));
...
fn test_set_cache_method() {
    ...
    assert_eq!(retrieved_config.prompt_cache_key, Some("pk-def-456".to_string()));
}
...
fn test_provider_model_with_cache() {
    ...
    let retrieved_config = model.cache_config().unwrap();
    assert_eq!(retrieved_config.xai_conv_id, Some("conv-via-provider".to_string()));
    assert_eq!(retrieved_config.prompt_cache_key, Some("pk-via-provider".to_string()));
}
...
fn test_provider_grok_4_20_with_cache() {
    ...
    let cache_config = CacheConfig::new()
        .with_xai_conv_id("conv-grok-420");

    let model = provider.grok_4_20_reasoning_with_cache(cache_config);

    let retrieved_config = model.cache_config().unwrap();
    assert_eq!(retrieved_config.xai_conv_id, Some("conv-grok-420".to_string()));
}
...
fn test_provider_grok_4_with_cache() {
    ...
    let cache_config = CacheConfig::new()
        .with_xai_conv_id("conv-grok-4");

    let model = provider.grok_4_with_cache(cache_config);

    let retrieved_config = model.cache_config().unwrap();
    assert_eq!(retrieved_config.xai_conv_id, Some("conv-grok-4".to_string()));
}
