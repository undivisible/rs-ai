use rs_ai_core::{AiError, AiResult, ModelInfo, Provider};

use super::api_types::OllamaListResponse;
...
impl Provider for OllamaProvider {
    ...
    fn language_model(&self, model_id: &str) -> AiResult<Box<dyn rs_ai_core::LanguageModel>> {
        Ok(Box::new(self.model(model_id)))
    }

    fn embedding_model(&self, model_id: &str) -> AiResult<Box<dyn rs_ai_core::EmbeddingModel>> {
        Ok(Box::new(self.model(model_id)))
    }
