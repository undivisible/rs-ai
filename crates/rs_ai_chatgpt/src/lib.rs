...
impl ChatGptProvider {
    ...
    /// let provider = rs_ai_chatgpt::ChatGptProvider::new(std::env::var("OPENAI_API_KEY")?);
    /// let mut session = provider.realtime_session(rs_ai_chatgpt::GPT_4O_REALTIME).await?;
    /// session.send_text("Say hello").await?;
    /// while let Some(ev) = session.recv().await {
    ///     if let Ok(rs_ai_chatgpt::realtime_api::ServerEvent::TextDelta { delta, .. }) = ev {
    ///         print!("{delta}");
    ///     }
    /// }
    /// # Ok(())
