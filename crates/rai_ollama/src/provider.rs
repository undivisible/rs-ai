use rai_ai::{AiError, AiResult, ModelInfo, Provider};
...
impl Provider for OllamaProvider {
    ...
    fn language_model(&self, model_id: &str) -> AiResult<Box<dyn rai_ai::LanguageModel>> {
        Ok(Box::new(self.model(model_id)))
    }

    fn embedding_model(&self, model_id: &str) -> AiResult<Box<dyn rai_ai::EmbeddingModel>> {
        Ok(Box::new(self.model(model_id)))
    }
